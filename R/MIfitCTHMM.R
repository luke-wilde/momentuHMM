...
#' Fit continuous-time multivariate HMMs to multiple imputation data
#' 
...
#' @export
MIfitCTHMM <- function(miData, ...) {
  UseMethod("MIfitCTHMM")
}

#' @rdname MIfitCTHMM
#' @method MIfitCTHMM default
#' @param nSims Number of imputations in which to fit the HMM using \code{\link{fitCTHMM}}. If \code{miData} is a list of \code{momentuHMMData} 
...
#' @param Time.name Character string indicating name of the time column. Default: 'time'.
...
#' @param nbStates Number of states of the HMM. See \code{\link{fitCTHMM}}.
#' @param dist A named list indicating the probability distributions of the data streams. See \code{\link{fitCTHMM}}.
...
#' @param beta0 Initial matrix of regression coefficients for the transition probabilities. See \code{\link{fitCTHMM}}. \code{beta0} may also be a list of length \code{nSims}, where each element 
#' is an initial matrix of regression coefficients for the transition probabilities for each imputation.
#' @param delta0 Initial values for the initial distribution of the HMM. See \code{\link{fitCTHMM}}. \code{delta0} may also be a list of length \code{nSims}, where each element 
...
#' @param formula Regression formula for the transition probability covariates. See \code{\link{fitCTHMM}}.
#' @param formulaDelta Regression formula for the initial distribution. See \code{\link{fitCTHMM}}.
...
#' @param formulaPi Regression formula for the mixture distribution probabilities. See \code{\link{fitCTHMM}}.
...
#' @param fit \code{TRUE} if the HMM should be fitted to the data, \code{FALSE} otherwise. See \code{\link{fitCTHMM}}. If \code{fit=FALSE} and \code{miData} is a \code{\link{crwData}}
...
#' @param fixPar An optional list of vectors indicating parameters which are assumed known prior to fitting the model. See \code{\link{fitCTHMM}}. 
...
#' @param retrySD An optional list of scalars or vectors indicating the standard deviation to use for normal perturbations of each working scale parameter when \code{retryFits>0}. See \code{\link{fitCTHMM}}.
...
#' 
#' @seealso \code{\link{crawlWrap}}, \code{\link[crawl]{crwPostIS}}, \code{\link[crawl]{crwSimulator}}, \code{\link{fitCTHMM}}, \code{\link{getParDM}}, \code{\link{MIpool}}, \code{\link{prepData}} 
#' 
#' @references
#' 
...
MIfitCTHMM.default<-function(miData,nSims, ncores = 1, poolEstimates = TRUE, alpha = 0.95, na.rm = FALSE,
                   Time.name="time",Time.unit="auto",nbStates, dist, 
                   ...
                   fullPost = TRUE, dfPostIS = Inf, scalePostIS = 1,thetaSamp = NULL, ...)
{
  ...
  # for directing list of momentuHierHMMData objects to MIfitCTHMM.hierarchical
  hierArgs <- list(...)
  ...
  if(is.crwData(miData)){
    ...
    if(nSims>=1) {
      ...
      if(fit) cat('Fitting',length(ind),'realizations of the position process using fitCTHMM... \n')
      else return(crwSim(list(miData=miData,crwSimulator=crwSim)))
    } else stop("nSims must be >0")
    
  } else {
    if(!is.list(miData)) stop("miData must either be a crwData object (as returned by crawlWrap) or a list of momentuHMMData objects as returned by simData, prepData, or MIfitCTHMM (when fit=FALSE)")
    if(is.crwSim(miData)) miData <- miData$miData
    ind <- which(unlist(lapply(miData,function(x) inherits(x,"momentuHMMData"))))
    if(!length(ind)) stop("miData must either be a crwData object (as returned by crawlWrap) or a list of momentuHMMData objects as returned by simData, prepData, or MIfitCTHMM (when fit=FALSE)")
    ...
    cat('Fitting',min(nSims,length(ind)),'imputation(s) using fitCTHMM... \n')
  }
  ...
  withCallingHandlers(fits[parallelStart:nSims] <-
    foreach(mD = miData[parallelStart:nSims], j = parallelStart:nSims, .export=c("fitCTHMM.momentuHMMData"), .errorhandling="pass") %dorng% {
      
      if(nSims>1) {
        cat("     \rImputation ",j,"... ",sep="")
      }
      tmpFit<-suppressMessages(fitCTHMM.momentuHMMData(mD, Time.name, Time.unit, nbStates, dist, Par0[[j]], beta0[[j]], delta0[[j]],
                                      estAngleMean, circularAngleMean, formula, formulaDelta, stationary, mixtures, formulaPi,
                                      nlmPar, fit, DM,
...
#' @param hierStates A hierarchical model structure \code{\link[data.tree]{Node}} for the states.  See \code{\link{fitCTHMM}}.
#' @param hierDist A hierarchical data structure \code{\link[data.tree]{Node}} for the data streams. See \code{\link{fitCTHMM}}.
#' @param hierBeta A hierarchical data structure \code{\link[data.tree]{Node}} for the matrix of initial values for the regression coefficients of the transition probabilities at each level of the hierarchy ('beta'). See \code{\link{fitCTHMM}}. 
#' @param hierDelta A hierarchical data structure \code{\link[data.tree]{Node}} for the matrix of initial values for the regression coefficients of the initial distribution at each level of the hierarchy ('delta'). See \code{\link{fitCTHMM}}. 
#' @param hierFormula A hierarchical formula structure for the transition probability covariates for each level of the hierarchy. See \code{\link{fitCTHMM}}.
#' @param hierFormulaDelta A hierarchical formula structure for the initial distribution covariates for each level of the hierarchy ('formulaDelta'). Default: \code{NULL} (no covariate effects and \code{fixPar$delta} is specified on the working scale). See \code{\link{fitCTHMM}}.
#' 
#' @export
#' @importFrom crawl crwPostIS crwSimulator
#' @importFrom doParallel registerDoParallel stopImplicitCluster
                       ...
                       fullPost = TRUE, dfPostIS = Inf, scalePostIS = 1,thetaSamp = NULL, ...)
{
  ...
  if(is.crwHierData(miData)){
    ...
    if(!is.list(miData)) stop("miData must either be a crwHierData object (as returned by crawlWrap) or a list of momentuHierHMMData objects as returned by simHierData, prepData, or MIfitCTHMM (when fit=FALSE)")
    if(is.crwHierSim(miData)) miData <- miData$miData
    ind <- which(unlist(lapply(miData,function(x) inherits(x,"momentuHierHMMData"))))
    if(!length(ind)) stop("miData must either be a crwHierData object (as returned by crawlWrap) or a list of momentuHierHMMData objects as returned by simHierData, prepData, or MIfitCTHMM (when fit=FALSE)")
    if(missing(nSims)) nSims <- length(miData)
    if(nSims>length(miData)) stop("nSims is greater than the length of miData. nSims must be <=",length(miData))
    if(nSims<1) stop("nSims must be >0")
    cat('Fitting',min(nSims,length(ind)),'imputation(s) using fitCTHMM... \n')
  ...
  registerDoParallel(cores=ncores)
  withCallingHandlers(fits[parallelStart:nSims] <-
                        foreach(mD = miData[parallelStart:nSims], j = parallelStart:nSims, .export=c("fitCTHMM.momentuHierHMMData"), .errorhandling="pass") %dorng% {
